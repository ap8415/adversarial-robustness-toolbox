...
from models.utils import load_mnist, load_mnist_vectorized, load_cifar10, load_spambase
...
l1_regularization_levels = [0, 1e-7, 2e-7, 3e-7, 5e-7, 7.5e-7, 1e-6, 2.5e-6, 5e-6, 1e-5, 2.5e-5, 5e-5, 1e-4, 2e-4]
...
for l1 in l1_regularization_levels:
    l1_all = []
    l2_all = []
    linf_all = []
    ...
    for i in range(5):
        ...
        if args.experiment_type == "three_layer":
            model = deep.three_layer_dnn(input_shape, output_shape, 0, l1, 0)
        elif args.experiment_type == "five_layer":
            model = deep.five_layer_dnn_model_wide(input_shape, output_shape, 0, l1, 0)
        elif args.experiment_type == "six_layer":
            model = deep.six_layer_dnn_model_wide(input_shape, output_shape, 0, l1, 0)
...
dataset_map = {'mnist': 'MNIST', 'cifar': 'CIFAR10', 'spambase': 'Spambase'}

l1_regularization_levels = [log(x) for x in l1_regularization_levels]

if args.attack_type == 'l2':
    plt.plot(l1_regularization_levels, l2_misclas_min, linestyle='--', marker='o')
else:
    plt.plot(l1_regularization_levels, linf_misclas_min, linestyle='--', marker='o')

if args.attack_type == 'l2':
    plt.plot(l1_regularization_levels, l2_misclas, linestyle='--', marker='o')
else:
    plt.plot(l1_regularization_levels, linf_misclas, linestyle='--', marker='o')

if args.attack_type == 'l2':
    plt.plot(l1_regularization_levels, l2_misclas_max, linestyle='--', marker='o')
else:
    plt.plot(l1_regularization_levels, linf_misclas_max, linestyle='--', marker='o')

figname = '%s, L1 reg., untargeted %s attack on %s' % \
    (name_map[args.experiment_type], attack_map[args.attack_type], dataset_map[args.dataset])
fig.suptitle(figname)
plt.xlabel('L1 coef, log scale')
plt.ylabel('Security score')
plt.savefig('%s_%s_%s_l1.png' % (args.experiment_type, args.attack_type, args.dataset))
