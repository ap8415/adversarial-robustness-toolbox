...
parser.add_argument("experiment_type", help="The model type used by the experiment. "
                                            "Values: VGG, leNet.")
parser.add_argument("dropout_type", help="The style of dropout used. Values: pooling, dense.")
    ...
    for i in range(5):
        ...
        if args.experiment_type == "VGG":
            if args.dropout_type == 'pooling':
                model = convolutional.vgg_pooling(args.dataset, dropout, 0, 0)
            elif args.dropout_type == 'dense':
                model = convolutional.vgg_dense(args.dataset, dropout, 0, 0)
            else:
...
name_map = {'leNet': 'LeNet5', 'VGG': 'VGG'}
style_map = {'pooling': 'pooling layers', 'dense': 'dense layers'}
...
figname = '%s, dropout on %s, targeted %s attack on %s' % \
...
plt.savefig('%s_%s_%s_%s_targeted.png' % (args.experiment_type, args.dropout_type, args.attack_type, args.dataset))