...
for dropout in dr:
    ...
    adversarial = attack(x_test[:10], np.argmax(y_test[:10], axis=1), iterations=60)
    ...
    if len(orig_examples_failed) > 0:
        adversarial_strong = attack(orig_examples_failed, np.argmax(correct_labels_failed, axis=1), iterations=150)
