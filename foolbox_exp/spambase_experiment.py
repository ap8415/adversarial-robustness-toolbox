...
from experiment_models import neural_networks_spambase
...
from art.utils import load_mnist, load_mnist_vectorized, load_spambase
...
for dropout in dr:
    ...
    if args.experiment_type == "three_layer_dnn":
        kmodel = neural_networks_spambase.three_layer_dnn_foolbox(x_train.shape[1:], 300, 100, dropout, 0, 0)
    elif args.experiment_type == "five_layer_dnn":
        kmodel = neural_networks_spambase.symmetric_five_layer_nn_foolbox(x_train.shape[1:], dropout, dropout)
    elif args.experiment_type == "six_layer_dnn":
        kmodel = neural_networks_spambase.asymmetric_six_layer_nn_foolbox(x_train.shape[1:], dropout, dropout)

    # kmodel.fit(x_train, y_train, epochs=1, batch_size=128)
    kmodel.fit(x_train, y_train, epochs=200, batch_size=64)
